...
from queue_spec import (
    ...
    JOB_REMOVE_FROM_SCHEDULE, JOB_RETARGET_CAPTURE, JOB_SET_EMPLOYEE_CONTACT, JOB_SET_EMPLOYEE_HOME_ADDRESS, JOB_SET_EMPLOYEE_ID, JOB_SET_EMPLOYEE_UNIFORM, JOB_SHIFT_REPORT_NOTE, JOB_TRIGGER_RECRUITING,
...
def target_path_hint(job: QueueJob, context: RunContext) -> str:
    ...
    try:
        ...
        if job.job_type == JOB_SET_EMPLOYEE_HOME_ADDRESS:
            return _canonical_employee_hint(payload["person"])
        if job.job_type == JOB_SET_EMPLOYEE_UNIFORM:
            return _canonical_employee_hint(payload["person"])
