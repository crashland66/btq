...
from queue_processor.main import DEFAULT_PROJECT_ROOT, DEFAULT_VAULT_ROOT, ensure_local_runtime_root, process_all
...
def run_once(
    ...
    logger.info(
        "processing queue project_root=%s vault_root=%s runtime_root=%s",
        project_root,
        vault_root,
        runtime_root,
    )
    process_all(project_root=project_root, vault_root=vault_root, runtime_root=runtime_root, dry_run=False)
...
def run(argv: list[str] | None = None) -> int:
    ...
    if args.once:
        run_once(project_root, vault_root, runtime_root, DEFAULT_CONFIG.local_root, DEFAULT_CONFIG.logs_dir, logger)
        return 0

    logger.info(
        "watching queue project_root=%s vault_root=%s runtime_root=%s interval=%.1fs",
        project_root,
        vault_root,
        runtime_root,
        args.poll_seconds,
