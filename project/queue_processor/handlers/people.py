...
# Personnel-event and availability-constraint handlers moved to their own
# domain module; re-exported here because registry, main, and tests resolve
# them through `people`.
from .personnel_events import (  # noqa: F401
    _build_personnel_event_entity_doc,
    _canonical_availability_person,
    availability_constraint_doc_id,
    availability_constraint_id,
    personnel_event_doc_id,
    personnel_event_id,
    process_log_availability_constraint_job,
...
def _build_employee_entity_doc(payload: dict, job: QueueJob, person_id: str, created_date: str) -> dict:
    ...
    home_address = _shared.normalized_home_address(payload.get("home_address"))
    if home_address:
        doc["home_address"] = home_address
    else:
        doc.pop("home_address", None)
