...
def render_list(root: Path, query: dict[str, list[str]]) -> str:
    ...
    all_columns = [
        ...
        {"key": "token_type", "label": "Token Type", "format": lambda value, _row: html.escape(humanize_key(value)), "priority": 2, "nowrap": True},
        ...
        {"key": "last_used", "label": "Last Used", "format": lambda value, row: f'<span class="pill warning">{html.escape(str(value))}</span>' if row.get("last_used_warning") else html.escape(str(value)), "priority": 2, "nowrap": True},
