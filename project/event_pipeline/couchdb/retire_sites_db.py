...
"""Archive the retired btq_sites database on one CouchDB node.

Run once per peer that still carries the database (Pro and VPS; the Dell too
if it re-joins the mesh with a btq_sites copy). Per node it:

  1. Deletes every ``_replicator`` doc that references btq_sites — both the
     legacy ``pro_to_vps_btq_sites`` naming and the mesh
     ``btq_sites_pro_to_vps`` naming, plus any opaque-id doc whose
     source/target URL points at the database.
  2. Archives the database with a local one-shot replication to
     ``btq_sites_bak_<date>`` (rename-by-copy; CouchDB has no rename).
  3. Verifies the archive's doc count matches, then deletes btq_sites.

Dry-run by default; ``--execute`` to apply. Point ``--url`` at the peer
(default is this node's instance config), e.g. from the Pro:

    python3 -m event_pipeline.couchdb.retire_sites_db --execute
    python3 -m event_pipeline.couchdb.retire_sites_db \
        --url http://<vps-tailnet-ip>:5984 --execute

Prerequisites: migrate_sites_to_vault has run (registration folded onto the
vault location docs, sites_by_alias/sites_by_site_id views registered) and
the repointed readers are deployed — nothing may read btq_sites anymore.
"""
...
class RetireSitesDBError(Exception):
    pass
...
def references_sites_db(doc: dict[str, Any]) -> bool:
    """True when a _replicator doc replicates btq_sites (by id or endpoint)."""
    doc_id = str(doc.get("_id") or "")
    # Match the live db in any naming era, but never the btq_sites_bak_*
    # archives (whose ids contain the same substring).
    if re.search(rf"{SITES_DB}(?!_bak)", doc_id):
        return True
    for endpoint in (doc.get("source"), doc.get("target")):
        path = parse.urlparse(_endpoint_url(endpoint)).path
        if path.rstrip("/").endswith(f"/{SITES_DB}"):
            return True
    return False
...
def _request_json(base_url: str, method: str, path: str, payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        raise RetireSitesDBError(f"CouchDB {method} {path} failed with HTTP {exc.code}: {detail}") from exc
...
def retire(base_url: str, *, execute: bool, archive_name: str) -> None:
    ...
    stale = [
        ...
        and references_sites_db(row["doc"])
    ]
    for doc in stale:
    ...
    info = _request_json(base_url, "GET", SITES_DB)
    doc_count = int(info.get("doc_count") or 0)
    print(f"[{mode}] archive {SITES_DB} ({doc_count} docs) -> {archive_name}")
    if execute:
        ...
        _request_json(base_url, "POST", "_replicate", {
            "source": {"url": f"{base_url.rstrip('/')}/{SITES_DB}", "headers": auth_headers},
        ...
        if archived != doc_count:
            raise RetireSitesDBError(
                f"Archive doc count mismatch: {archive_name} has {archived}, expected {doc_count} — "
                f"{SITES_DB} NOT deleted"
            )

    # 3. Delete the original.
    print(f"[{mode}] delete {SITES_DB}")
    if execute:
        _request_json(base_url, "DELETE", SITES_DB)
    print(f"[{mode}] done")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--archive-name",
        default=f"btq_sites_bak_{date.today().strftime('%Y%m%d')}",
        help="Archive database name (default: btq_sites_bak_<today>).",
    )
    args = parser.parse_args(argv)
    base_url = args.url or couchdb_config.base_url()
    try:
        retire(base_url, execute=args.execute, archive_name=args.archive_name)
    except RetireSitesDBError as exc:
