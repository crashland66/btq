...
def test_review_dashboard_includes_maintenance_findings_and_suggests_maintenance(
    ...
    assert not (runtime_root / "failed").exists()
    assert sentinel.read_text(encoding="utf-8") == "do not touch\n"
def test_supply_after_photo_duplicate_is_suppressed(tmp_path: Path) -> None:
    semantic_dir = tmp_path / "runtime" / "field_capture" / "audio_semantics"
...
def test_junk_test_audio_is_suppressed(tmp_path: Path) -> None:
    ...
    # All summaries are suppressed by quality rules -> no candidate payloads.
    assert field_action_candidates.payloads_from_semantic(semantic_path, payload) == []
